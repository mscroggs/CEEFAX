...
class CuPT:
    ...
    def reveal(self):
        ...
        for y in self.show_cls:
            for x in self.show_cls[y]:
                try:
                    pad.addstr(0, 0, self.show_cls[y][x][0],
                               self.show_cls[y][x][1])
                except:
                    try:
                        pad.addstr(0, 0,
                                   self.show_cls[y][x][0].encode("utf-8"),
                                   self.show_cls[y][x][1])
                    except:
                        pad.addstr(0, 0, " ")
                try:
    ...
    def unreveal(self):
        ...
        for y in self.unshow_cls:
            for x in self.unshow_cls[y]:
                try:
                    pad.addstr(0, 0, self.unshow_cls[y][x][0],
                               self.unshow_cls[y][x][1])
                except:
                    try:
                        pad.addstr(0, 0,
                                   self.unshow_cls[y][x][0].encode("utf-8"),
                                   self.unshow_cls[y][x][1])
                    except:
                        pad.addstr(0, 0, " ")
                try:
        ...
        for n in sorted(tran.keys()):
            for y, x in tran[n]:
                if y in self.cls and x in self.cls[y]:
                    try:
                        pad.addstr(0, 0, self.cls[y][x][0],
                                   self.cls[y][x][1])
                    except:
                        try:
                            pad.addstr(0, 0,
                                       self.cls[y][x][0].encode("utf-8"),
                                       self.cls[y][x][1])
                        except:
                            pad.addstr(0, 0, " ")
                else:
...
curses_colors = {"RED": (curses.COLOR_RED, 0),
...
html_colors = {"RED": "Red", "LIGHTRED": "LightCoral", "GREEN": "Green",
               "LIGHTGREEN": "LightGreen", "BLUE": "Blue",
               "LIGHTBLUE": "SkyBlue", "YELLOW": "Yellow", "CYAN": "Cyan",
               "LIGHTCYAN": "Aquamarine", "ORANGE": "Orange",
               "MAGENTA": "Magenta", "PINK": "Pink", "GREY": "Gray",
               "WHITE": "White", "BRIGHTWHITE": "White", "BLACK": "Black"}

non_dark_colors = ["RED", "LIGHTRED", "GREEN", "LIGHTGREEN", "BLUE",
                   "LIGHTBLUE", "YELLOW", "CYAN", "LIGHTCYAN", "ORANGE",
                   "MAGENTA", "PINK", "WHITE", "BRIGHTWHITE"]
